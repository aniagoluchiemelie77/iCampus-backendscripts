...
export const ValidateInstitution = async (req, res) => {
  try {
    ...
    if (!schoolName) return res.status(400).json({ message: "School name required" });
    ...
    if (!institution) {
      return res.status(404).json({
        verified: false,
        message: "iCampus not yet operational in this institution. Student/Lecturer verification is unavailable.",
      });
    }
