  ...
  test.each(endpointsToTest)(
    ...
    async ({ method, path, auth, expected: expectedStatus }) => {
      ...
      if (response.statusCode !== expectedStatus) {
        const errorDetails = response.body?.message || response.body?.error || response.text || "No error body provided";
        console.error(`❌ [MISMATCH] /${path} expected ${expectedStatus}, got ${response.statusCode}. Backend message:`, errorDetails);
      }
